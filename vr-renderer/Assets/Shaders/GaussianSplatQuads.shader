Shader "Unlit/GaussianSplatQuads"
{
    Properties
    {
        _Opacity ("Global Opacity", Range(0,1)) = 0.55
        _SigmaCutoff ("Sigma Cutoff (k)", Float) = 2.6
        _CutoffSoftness ("Cutoff Softness", Range(0.01,1.0)) = 0.5
        _MinAxisPixels ("Min Footprint (px)", Float) = 0.75
        _MaxAxisPixels ("Max Footprint (px)", Float) = 48.0
        _DepthAwareStrength ("Depth-aware Strength", Range(0,8)) = 3.0
        _DepthAwareBias ("Depth-aware Bias (m)", Float) = 0.02
        _SurfaceThickness ("Surface Thickness (m)", Float) = 0.08
        _SurfaceMinAlpha  ("Surface Min Alpha", Range(0,0.5)) = 0.04
        _GrazeBoostMax ("Grazing Boost Max", Range(1,4)) = 1.15
    }
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            FragmentOutput frag(Varyings i)
            {
                ...
                // Reject only clearly out-of-support fragments; the visible edge is softened below.
                if (s > cutoff2) discard;

                // Gaussian falloff across the whole splat support.
                float alpha = exp(-0.5 * s) * _Opacity;

                // Replace the hard k-sigma edge with a soft fade near the boundary.
                float softness = saturate(_CutoffSoftness);
                float fadeStart = cutoff2 * (1.0 - softness);
                float edgeFade = 1.0 - smoothstep(fadeStart, cutoff2, s);
                alpha *= edgeFade;
        ...
        Pass
        {
            ...
            static const float  kNearCompMin         = 1.0;
            static const float  kNearCompMax         = 1.0;
            static const float  kDepthAwareScale     = 15.0;
            static const float  kViewZFadeEnabled    = 0.0;
            static const float  kViewZFadeStart      = 0.0;
            static const float  kViewZFadeEnd        = 10.0;
            static const float  kViewZFadeExponent   = 1.0;
            ...
            struct Varyings
            {
                float4 positionCS : SV_POSITION;
                float4 color      : COLOR;
                float2 dpix       : TEXCOORD0;
                float4 qAndCut    : TEXCOORD1;
                float  viewZ      : TEXCOORD2;
            };
            ...
            float4 fragDirect(Varyings i) : SV_Target
            {
                float2 d = i.dpix;
                float q00 = i.qAndCut.x;
                float q01 = i.qAndCut.y;
