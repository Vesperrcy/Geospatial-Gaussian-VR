...
# MacBook Air M2 default: reduce memory pressure while keeping flow unchanged.
MAX_GAUSSIANS = 1_200_000
MAX_GAUSSIANS_AFTER_SAMPLING = MAX_GAUSSIANS

# M2 adaptive controls
AUTO_PROFILE_M2 = True
AUTO_TUNE_M2 = True
M2_AUTOTUNE_PROBE_POINTS = 30000
CKDTREE_WORKERS = -1
...
def choose_m2_profile(preset: str, total_mem_gb: float, cpu_count: int) -> tuple[int, int, int]:
    p = (preset or "verify").lower().strip()

    if total_mem_gb <= 8.5:
        verify_max, final_max, block, workers = 850_000, 1_500_000, 8000, min(4, max(2, cpu_count))
    elif total_mem_gb <= 16.5:
        verify_max, final_max, block, workers = 1_400_000, 2_600_000, 14000, min(6, max(3, cpu_count))
    else:
        verify_max, final_max, block, workers = 2_200_000, 3_800_000, 20000, min(8, max(4, cpu_count))

    max_points = final_max if p == "final" else verify_max
    return int(max_points), int(block), int(max(1, workers))


def apply_m2_profile_if_needed() -> None:
    global MAX_GAUSSIANS, MAX_GAUSSIANS_AFTER_SAMPLING
    global ANISO_BLOCK_SIZE, CKDTREE_WORKERS
    global USER_OVERRIDE_MAX_POINTS, USER_OVERRIDE_CKDTREE_WORKERS

    if not AUTO_PROFILE_M2 or not is_apple_silicon_mac():
        return

    mem_gb = detect_total_memory_gb()
    cpu_count = os.cpu_count() or 8
    max_points, block, workers = choose_m2_profile(PRESET, mem_gb, cpu_count)

    if not USER_OVERRIDE_MAX_POINTS:
        MAX_GAUSSIANS = int(max_points)
        MAX_GAUSSIANS_AFTER_SAMPLING = MAX_GAUSSIANS
    ANISO_BLOCK_SIZE = int(block)
    if not USER_OVERRIDE_CKDTREE_WORKERS:
        CKDTREE_WORKERS = int(workers)

    log(f"[INFO] M2 profile enabled: RAM={mem_gb:.1f} GB, CPU={cpu_count} logical cores")
    log(f"[INFO] M2 profile params: max_points={MAX_GAUSSIANS}, aniso_block={ANISO_BLOCK_SIZE}, cKDTree_workers={CKDTREE_WORKERS}")
...
def autotune_m2_aniso_params(points: np.ndarray,
                             scales_iso: np.ndarray,
                             normals: np.ndarray,
                             current_block: int,
                             current_workers: int) -> tuple[int, int]:
    if not (AUTO_TUNE_M2 and is_apple_silicon_mac() and _HAS_SCIPY):
        return int(current_block), int(current_workers)

    n = int(points.shape[0])
    probe_n = int(min(max(12000, M2_AUTOTUNE_PROBE_POINTS), n))
    if probe_n < 12000:
        return int(current_block), int(current_workers)

    idx = np.linspace(0, n - 1, probe_n, dtype=np.int64)
    p_probe = points[idx]
    s_probe = scales_iso[idx]
    n_probe = normals[idx]

    worker_candidates = sorted({max(1, int(current_workers)), max(1, int(current_workers - 2)), max(1, int(current_workers + 2))})
    block_candidates = sorted({max(4000, int(current_block)), max(4000, int(current_block * 3 // 4)), max(4000, int(current_block * 5 // 4))})
    ...
    for w in worker_candidates:
        for b in block_candidates:
            ...
            try:
                compute_normal_aligned_gaussians_batched(
                    points=p_probe,
                    scales_iso=s_probe,
                    normals=n_probe,
                ...
                speed = probe_n / dt
                log(f"[INFO] M2 autotune probe: workers={w}, block={b}, speed={speed:.1f} pts/s")
                if (best is None) or (speed > best[0]):
                    best = (speed, b, w)
            except Exception as e:
                log(f"[WARN] M2 autotune probe failed (workers={w}, block={b}): {e}")

    if best is None:
        return int(current_block), int(current_workers)

    _, best_block, best_workers = best
    log(f"[INFO] M2 autotune selected: workers={best_workers}, block={best_block}")
    return int(best_block), int(best_workers)
...
def compute_normal_aligned_gaussians_batched(points: np.ndarray,
                                             ...
                                             query_workers: int = -1) -> tuple[np.ndarray, np.ndarray, np.ndarray]:
    if not _HAS_SCIPY:
    ...
    for b0 in range(0, N, int(block_size)):
        ...
        d, idx = _tree_query(tree, p, k=k, workers=query_workers)
...
def main():
    ...
    if AUTO_TUNE_M2 and is_apple_silicon_mac() and _HAS_SCIPY:
        ANISO_BLOCK_SIZE, CKDTREE_WORKERS = autotune_m2_aniso_params(
            points=points,
            scales_iso=scales_iso,
            normals=normals,
            current_block=ANISO_BLOCK_SIZE,
            current_workers=CKDTREE_WORKERS,
        )
...
if __name__ == "__main__":
    ...
    ap.add_argument("--ckdtree_workers", type=int, default=None, help="Override cKDTree query workers")

    ap.add_argument("--no_m2_profile", action="store_true", help="Disable M2 memory/core profile")
    ap.add_argument("--no_m2_autotune", action="store_true", help="Disable M2 probe autotuning")
    ap.add_argument("--m2_probe_points", type=int, default=M2_AUTOTUNE_PROBE_POINTS, help="Probe points for M2 autotune")
    ...
    AUTO_PROFILE_M2 = not bool(args.no_m2_profile)
    AUTO_TUNE_M2 = not bool(args.no_m2_autotune)
    M2_AUTOTUNE_PROBE_POINTS = int(max(10000, args.m2_probe_points))
